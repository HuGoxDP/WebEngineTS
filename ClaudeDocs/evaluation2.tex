...
\textbf{Reproducibility.} The measurement harness, the benchmark scenes, the exact engine build that was measured, the run protocol, and the raw per-run exports behind every table in this section are published at \url{https://github.com/<user>/webenginets-benchmarks}. The suite is self-contained: \texttt{npm install \&\& npm start} serves it locally, and each configuration is selected entirely through URL parameters, so every reported figure can be re-measured independently. Each exported run records its own configuration, the GPU that served it, and a timestamp, and the aggregation script recomputes the tables below from those raw files.
